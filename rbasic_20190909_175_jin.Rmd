...
```{r}
codes <- c(china = 123, korea = 234, US = 345)
```
```{r}
codes
```
...
data("murders")
```
1. Compute the per 100,000 murder rate for each state and store it in an object called **murder_rate**. Then use logical operators to create a logical vector named **low** that tells us which entries of **murder_rat**e are lower than 1.
```{r}
murder_rate <- murders$total/murders$population*100000
low <- murder_rate < 1
```
...
```{r}
ma <- match(c("AK", "MI", "IA"), murders$abb)
murders$state[ma]
```
...
```{r}
population_in_log <- log(murders$population, 10)
total_gun_murders <- murders$total
...
```
G3. enerate boxplots of the state populations by region.
```{r}
