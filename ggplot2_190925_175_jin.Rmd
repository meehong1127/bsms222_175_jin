...
```{r}
p + geom_point(size = 3,aes(col=region), show.legend = FALSE)
```
...
Here we want to add a line that represents the average murder rate for the entire country. Once we determine the per million rate to be  _r_, this line is defined by the formula:  y=rx , with  y  and x our axes: total murders and population in millions respectively. In the log-scale this line turns into:  log(y)=log(r)+log(x) . So in our plot it’s a line with slope 1 and intercept  log(r) . To compute this value, we use our **dplyr** skills:
```{r}
r <- murders %>% 
...
```{r}
p <- p + geom_abline(intercept = log10(r),lty = 'dotted', color = "darkgrey") +
