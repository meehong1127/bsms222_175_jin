...
![simulated-polls-1](https://rafalab.github.io/dsbook/book_files/figure-html/simulated-polls-1.png)
...
![confidence-coverage-2008-election-1](https://rafalab.github.io/dsbook/book_files/figure-html/confidence-coverage-2008-election-1.png)
...
![fivethirtyeight-densities-1](https://rafalab.github.io/dsbook/book_files/figure-html/fivethirtyeight-densities-1.png)
...
![pollster-bias-1](https://rafalab.github.io/dsbook/book_files/figure-html/pollster-bias-1.png)
...
Our task is to estimate $d$. Because we model the observed values $X_1,\dots X_N$ as a random sample from the urn, the CLT might still work in this situation because it is an average of independent random variables. For a large enough sample size $N$, the probability distribution of the sample average $\bar{X}$ is approximately normal with expected value $μ$ and standard error $\sigma/\sqrt{N}$. If we are willing to consider $N=15$ large enough, we can use this to construct confidence intervals.

A problem is that we don’t know $\sigma$. But theory tells us that we can estimate the urn model $\sigma$ with the _sample standard deviation_ defined as $s = \sqrt{ \sum_{i=1}^N (X_i - \bar{X})^2 / (N-1)}$.
...
s<-sd(X)
se_hat<-s/sqrt(N)
c( mu_hat- 1.96 * se_hat, mu_hat + 1.96 * se_hat)
```
...
a<-replicate(B, {
   ...
   s<-sd(X)
   se_hat<-s/sqrt(N)
   interval<-c( mu_hat- 1.96 * se_hat, mu_hat + 1.96 * se_hat)
   mu<-mean(x)
...
13.What is the standard error of $\bar{Y}_1$?
A: $\ \sigma_2/\sqrt{N_2}$
...
A: $\ \sigma_2/\sqrt{N_2}-\ \sigma_1/\sqrt{N_1}$
...
```{r}
2 * (1 - pnorm(e_hat / se_hat, 0, 1))
```

20.The statistic formed by dividing our estimate of $b_2−b1_$ by its estimated standard error:
...
polls%>%
  group_by(pollster)%>%
  summarize(ave=mean(spread), sd=sd(spread))
```
