...
In Section 8.4, we explained why when summarizing a list of numeric values, such as heights, it is not useful to construct a distribution that defines a proportion to each possible outcome. For example, if we measure every single person in a very large population of size  _n_ with extremely high precision, since no two people are exactly the same height, we need to assign the proportion  1/n  to each observed value and attain no useful summary at all. Similarly, when defining probability distributions, it is not useful to assign a very small probability to every single height.
...
which mathematically is the grey area below:(intergrals-1)[https://rafalab.github.io/dsbook/book_files/figure-html/intergrals-1.png]
...
Not surprisingly, the distribution looks normal:(simulated-heights-1)[https://rafalab.github.io/dsbook/book_files/figure-html/simulated-heights-1.png]
...
Here is the resulting distribution:(simulated-tallest-height-1)[https://rafalab.github.io/dsbook/book_files/figure-html/simulated-tallest-height-1.png]
...
taller<-m+s
shorter<-m-s
pnorm(taller, m, s) - pnorm(shorter, m, s)
```
