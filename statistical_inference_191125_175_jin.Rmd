...
![first-confidence-intervals-example-1](https://rafalab.github.io/dsbook/book_files/figure-html/first-confidence-intervals-example-1.png)
...
![confidence-interval-coverage-1](https://rafalab.github.io/dsbook/book_files/figure-html/confidence-interval-coverage-1.png)
...
```{r}
polls2<-polls%>%
  mutate(x_hat=(polls$d_hat+1)/2,se_hat=sqrt(x_hat * (1 - x_hat) /polls$samplesize ), lower=x_hat - qnorm(0.975) * se_hat, upper=x_hat + qnorm(0.975) * se_hat, hit=lower<0.482&upper>0.482)
```
...
```{r}
polls2%>%
  mutate(difference=d_hat-0.021)%>%
  ggplot(aes(pollster, difference))+
  geom_point()+
...
This does not mean that the election is close. It only means that we have a small sample size. In statistical textbooks this is called lack of _power_. In the context of polls, power is the probability of detecting spreads different from 0.
...
```{r}
career <- data_frame(result=c("win", "lose"), before=c(500*0.7-8,500*0.3-9), olympics= c(8,9))
chisq_test<-career%>%
  select(-result)%>%
  chisq.test()
chisq_test$p.value
...
```{r}
odd_olympics<-with(career, (olympics[2]/sum(olympics)) / (olympics[1]/sum(olympics)))
odd_before<-with(career, (before[2]/sum(before)) / (before[1]/sum(before)))
log_or<-log(odd_olympics/odd_before)
se <- career %>% select(-result) %>%
  summarize(se = sqrt(sum(1/olympics) + sum(1/before))) %>%
  pull(se)
ci <- log_or + c(-1,1) * qnorm(0.975) * se
exp(ci)
```
...
```{r}
career <- data_frame(result=c("win", "lose"), before=c((500*0.7-8)*2,(500*0.3-9)*2), olympics= c(8*2,9*2))
chisq_test<-career%>%
  select(-result)%>%
  chisq.test()
chisq_test$p.value
