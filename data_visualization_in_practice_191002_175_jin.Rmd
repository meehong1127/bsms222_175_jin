...
```{r}
a<-gapminder%>%
  filter(year == 2015 & country %in% c("South Korea","Turkey", "Russia","Vietnam", "South Africa")) %>% 
  pull(infant_mortality)
gapminder%>%
  filter(year == 2015 & country %in% c("Sri Lanka","Poland", "Malaysia", "Pakistan","Thailand")) %>% 
  select(country, infant_mortality)%>%
  mutate(country1=c("South Korea","Turkey", "Russia","Vietnam", "South Africa"), infant_mortality1=a)
```
...
library(scales)
surv_income%>%
  ggplot(aes(income,infant_survival_rate,col=group, label=group))+ geom_label(size=3)+
  scale_x_continuous(trans = log_trans(),breaks = c(1,8,64)) + 
  scale_y_continuous(breaks = c(0.850,0.900,0.950,0.990,0.995,0.998))
...
Jumping to this conclusion based on a plot showing averages is referred to as the _ecological fallacy_. The almost perfect relationship between survival rates and income is only observed for the averages at the region level. Once we show all the data, we see a somewhat more complicated story: [plot](https://rafalab.github.io/dsbook/book_files/figure-html/ecological-fallacy-all-data-1.png)
