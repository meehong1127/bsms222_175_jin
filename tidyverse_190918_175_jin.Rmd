...
inherits(ChickWeight, "data.frame") 
```
...
```{r}
nrow(murders[murders$region!="South",])
...
```{r}
filter(murders, region %in% c("Northeast", "West") & murder_rate<1)
```
...
```{r}
my_states<- filter(murders, region %in% c("Northeast", "West") & murder_rate<1)
select(my_states, state, murder_rate, rank)
```
## 5.5 The pipe: ```%>%```
...
```{r}
my_states <-filter(murders, region %in% c("Northeast", "West") & murder_rate<1) %>% select (state, rate, rank)
```
