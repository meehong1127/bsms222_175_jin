...
heights%>%
  ...
  geom_histogram(position="identity",aes(x=height,y=..density..),fill="white")+
  facet_wrap(.~sex)
```
...
From this plot, one would conclude that countries in Asia are much more populous than in other continents. Following the _show_ the data principle, we quickly notice that this is due to two very large countries, which we assume are India and China:
...
# ggplot(df, aes(x=cond, y=yval, fill=cond)) + geom_bar(stat="identity") + scale_fill_hue(c=45, l=80)
...
```{r}
# ggplot(df, aes(x=cond, y=yval, fill=cond)) + geom_bar(stat="identity") + scale_fill_manual(values=c("red", "blue", "green"))

# ggplot(df, aes(x=cond, y=yval, fill=cond)) + geom_bar(stat="identity") +scale_fill_manual(values=c("#CC6666", "#9999CC", "#66CC99"))
...
avg <- us_contagious_diseases %>%
  ...
  summarize(us_rate = sum(count, na.rm=TRUE)/sum(population, na.rm=TRUE)*10000)
us_contagious_diseases %>%
  filter(!state%in%c("Hawaii","Alaska") & disease == "Smallpox"& weeks_reporting>=10&!is.na(rate)) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
  mutate(state = reorder(state, rate)) %>%
    ggplot() +
  geom_line(aes(year, rate, group = state),  color = "grey50", 
...
```{r}
us_contagious_diseases %>%
  filter(state=="California" & weeks_reporting>=10 & !is.na(rate)) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
 ggplot(aes(year, rate, col = disease)) +
  geom_line() +
   xlab("") + 
  ylab("") +
  ggtitle("Cases per 10,000 by Disease")
```
