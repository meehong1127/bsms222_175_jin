...
2. The payout for winning on green is 17 dollars. This means that if you bet a dollar and it lands on green, you get 17 dollars. Create a sampling model using sample to simulate the random variable $X$ for your winnings. Hint: see the example below for how it should look like when betting on red.
```{r}
...
n<-1000
B<-1000
betting_green<-function(n){
  X <- sample(c(-1,17), n, replace = TRUE, prob=c(18/19, 1/19))
  sum(X)
}
S<-replicate(B, betting_green(n))
mean(S)
sd(S)
```
...
```{r}
SEY<-abs((17 - -1))*sqrt(18*1)/19/sqrt(n)
SEY
```
...
n<-1000
B<-2500
betting_average<-function(n){
  X <- sample(c(-1,17), n, replace = TRUE, prob=c(18/19, 1/19))
  mean(X)
}
Y<-replicate(B, betting_average(n))
mean(Y)
sd(Y)
```

17. Now check your answer to 15 using the Monte Carlo result.
```{r}
mean(Y>0)
```
...
with $Z$ a standard normal random variable as shown earlier. If we define $μ$ and $σ$ to be the expected value and standard deviation of the urn, respectively (that is of a single loan), using the formulas above we have: $\mbox{E}[S]= n\mu$ and  $\mbox{SE}[S]= \sqrt{n}\sigma$. So if we define  $z=$```qnorm(0.01)```, we have:
