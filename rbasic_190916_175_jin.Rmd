...
plot(n, s_n)
```
If you noticed that it appears to be a quadratic, you are on the right track because the formula is  n(n+)/2 , which we can confirm with a table:
```{r}
...
_n_, computes the sum of the integers from 1 to n (inclusive). Use the function to determine the sum of integers from 1 to 5,000.
```{r}
sum_n <- function(n){
  x<-1:n
  sum(x)
...
7. Write a function **compute_s_n** that for any given  n  computes the sum  . Report the value of the sum when  n=10 .
```{r}
compute_s_n <- function(n){
  y<- 1:n
  z<- y^2
